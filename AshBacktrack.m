...
static NSUInteger BUFFER_COUNT = 100;
...
//            printf("\n --- thread_id : %llu --- \n",identifierInfo->thread_id);
        }
        
        thread_info_data_t threadInfo;
        thread_basic_info_t threadBasicInfo = NULL;
        mach_msg_type_number_t threadBasicInfoOutCount = THREAD_INFO_MAX;
        kern_return_t basicResult = thread_info(threadAct, THREAD_BASIC_INFO, (thread_info_t)&threadInfo, &threadBasicInfoOutCount);
        if (basicResult == KERN_SUCCESS) {
            threadBasicInfo = (thread_basic_info_t)threadInfo;
            if (!(threadBasicInfo->flags & TH_FLAGS_IDLE)) {
                // 若不是空闲线程
//                printf("\ncpu_usage : %d\n",threadBasicInfo->cpu_usage / 10);
//                printf("\nsystem_time.microseconds : %d\n",threadBasicInfo->system_time.microseconds);
            }
        }
        ...
        //         获取线程栈
        printf("\n --- start --- \n");
        threadStackBacktrack(threadAct);
        printf("\n --- end --- \n");
    }
}

void threadStackBacktrack(thread_act_t thread_act) {
    
    int frameCount = 0;
    uintptr_t buffer[BUFFER_COUNT];
    ...
    do {
        ...
        ++frameCount;
        vm_read_overwrite(mach_task_self(), (vm_address_t)stackFrame.previous, sizeof(stackFrame), (vm_address_t)&stackFrame, &outsize);
    } while (stackFrame.previous != 0 && frameCount < BUFFER_COUNT);
    
    for (int i = 0; i < frameCount; ++i) {
        machoImageWithAddress(buffer[i]);
    }
}

void machoImageWithAddress(uintptr_t address) {
